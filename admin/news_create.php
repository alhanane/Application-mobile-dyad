...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    try {
        ...
        $category = trim((string)($_POST['category'] ?? 'Information'));
        ...
        $pdfUrl = null;
        try {
            $pdfUrl = upload_file('pdf_file', 'news_pdfs', ['application/pdf'], ['pdf']);
        } catch (Throwable $e) {
        ...
        $stmt = $pdo->prepare(
            'INSERT INTO news (title, description, image_url, pdf_url, link_url, category, target_type, level_id, class_id, published_at, created_by)\n'
            . 'VALUES (:t,:d,:img,:pdf,:link,:cat,:tt,:lid,:cid,:pub,:cb)'
        );
        $stmt->execute([
            ':t' => $title,
            ':d' => $description,
        ...
        $titleNotif = 'Nouvelle actualité';
        $bodyNotif = $title;
        $data = ['kind' => 'news', 'id' => $id, 'deeplink' => '/news/' . $id];

        $sentCount = 0;
        foreach ($topics as $topic) {
            $resp = sendToTopic($topic, $titleNotif, $bodyNotif, $data);
            log_notification($pdo, 'news', $titleNotif, $bodyNotif, 'topic', $topic, null, $resp['status'] ?? null, $resp['response'] ?? null);
            $sentCount++;
        }

        admin_flash_set('info', 'Actualité publiée. Notifications envoyées sur ' . $sentCount . ' topic(s).');
...
admin_page_start('Créer une actu');

echo '<div class="grid"><div class="card">';
echo '<h2>Créer une Actualité</h2>';
echo '<p class="muted">Publie en base et déclenche un push FCM sur les topics. Ajoutez une catégorie pour aider au tri côté app.</p>';
...
echo '<div class="field"><label>Catégorie</label><input name="category" maxlength="40" placeholder="Information, Pédagogie..." value="' . htmlspecialchars((string)($_POST['category'] ?? 'Information'), ENT_QUOTES, 'UTF-8') . '"></div>';
echo '</div>';
